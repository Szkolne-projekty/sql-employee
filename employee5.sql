...
SELECT EMPLOYEE_ID, JOB_TITLE, DATEDIFF(END_DATE, START_DATE) AS LICZBA_DNI
FROM job_history
NATURAL JOIN jobs
...
SELECT  JOB_TITLE, FIRST_NAME, LAST_NAME, (SALARY - MIN_SALARY) AS ROZNICA_WYNAGRODZEN
FROM employees
NATURAL JOIN jobs;
...
SELECT e.EMPLOYEE_ID, e.FIRST_NAME, e.LAST_NAME, jh.JOB_ID, j.JOB_TITLE, jh.START_DATE, jh.END_DATE, jh.DEPARTMENT_ID
FROM employees e JOIN job_history jh ON e.EMPLOYEE_ID = jh.EMPLOYEE_ID JOIN jobs j ON j.JOB_ID = jh.JOB_ID
